...
public class eis
{
	...
	public static final String CONFIG_AUTO_KEY_FOLDER_NAME = "holder\\";
	...
	public static void main(String[] args) throws Exception
	{
		...
		case "-keygenerate":
			...
			break;
		case "-encrypt":
				configManualKeyFront = args[1];
				configManualKeyRear = args[2];
				enCrypt(args[3], args[4]);
			break;
		case "-decrypt":
				configManualKeyFront = args[1];
				configManualKeyRear = args[2];
				deCrypt(args[3], args[4]);
			break;
		}
	}
	...
	public static void enCrypt(final String IMPORT_FILE_PATH, final String EXPORT_DIRECTORY_PATH) throws Exception
	{
		...
		while((bufferLimit = bis.read(buffer)) != -1)
		{
			for(int lap = 0; lap < bufferLimit; lap++)
			{
				...
				RandomAccessFile raf = new RandomAccessFile(CONFIG_AUTO_KEY_FOLDER_NAME + (configManualKeyFrontHash[hashCounter] + 128) + CONFIG_AUTO_KEY_FILE_EXTENSION, "r");
